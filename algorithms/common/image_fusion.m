function image_fusion(image_1, image_2, solution, saveName)
    ...
    % 裁剪冗余区域
    left_up=(solution_1*solution)*[1,1,1]';
    left_down=(solution_1*solution)*[1,M2,1]';
    right_up=(solution_1*solution)*[N2,1,1]';
    ...
    if(X_min>N1+1), X_min=N1+1; end
    if(X_max<2*N1), X_max=2*N1; end
    if(Y_min>M1+1), Y_min=M1+1; end
    if(Y_max<2*M1), Y_max=2*M1; end
    ...
    hold off;
    set(gca, 'LooseInset', get(gca, 'TightInset')); % 去掉白边

    % 如果传入保存路径，则保存 600 dpi
    if nargin==4 && ~isempty(saveName)
        print(saveName, '-dpng', '-r600'); % 600 dpi
        close; % 保存后关闭窗口
    end
end
